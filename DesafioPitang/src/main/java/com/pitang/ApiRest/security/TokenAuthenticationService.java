...
public class TokenAuthenticationService {
	...
	public static String getToken(Usuario usuario) {
		...
		String jwt = Jwts.builder().setSubject(usuario.getFirstName())
				.setExpiration(new Date(System.currentTimeMillis() + EXPIRATION_TIME))
				.signWith(SignatureAlgorithm.HS512, SECRET).compact();
	...
	public static String getNome(String token) {
		...
		String resposta = Jwts.parser().setSigningKey(SECRET).parseClaimsJws(token.replace(TOKEN_PREFIX, "")).getBody()
				.getSubject();
		return resposta;
	}
